...
export const ProjectsDropdown = (props: DropdownProps) => {
  ...
  function clickMenuItem(id: string) {
    const selectedProject = props.projectsList.find(item => item.id === id);
    if (!selectedProject) {
      return;
    }
  ...
  return (
    <div className={style.projectSelectPopup}>
      <button
        ...
        ref={buttonRef}
        aria-label={props.buttonAriaLabel ?? 'Выбор проекта для новой задачи'}
      >
        <svg
          className={style.projectSelect__icon}
          viewBox="0 0 16 16"
          fill="none"
